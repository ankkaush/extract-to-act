...
**Phase 11 of 19 — Approval Workflow — complete.** An uploaded document is extracted (Mistral OCR), then deterministically validated — required fields, arithmetic consistency, vendor matching, and duplicate detection — before landing in `VALIDATED`, `NEEDS_REVIEW`, or `DUPLICATE`, all in one synchronous request. A `NEEDS_REVIEW` document goes to a real queue (`GET /review`, `POST /review/{id}/correct`/`reject` — see `app/review.py`). Separately, a `VALIDATED` document at or above a configured dollar threshold needs a person's sign-off before anything downstream happens to it: `GET /approvals` lists what's waiting, `POST /approvals/{id}/approve` records the decision without changing the document's state (it stays `VALIDATED` — the approval record itself is the signal a later worker will check), and `POST /approvals/{id}/reject` moves it to `REJECTED`. Nothing looks at AI confidence to decide any of this; see [`app/validation.py`](app/validation.py), [`app/vendor_matching.py`](app/vendor_matching.py), [`app/duplicate_detection.py`](app/duplicate_detection.py), [`app/review.py`](app/review.py), and [`app/approval.py`](app/approval.py). The actual downstream accounting write doesn't exist yet — approved/auto-cleared documents currently just stay `VALIDATED`. See [`PLAN.md`](PLAN.md) for the full phase roadmap and current progress.

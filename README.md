...
**Phase 10 of 19 — Human Review Workflow — complete.** An uploaded document is extracted (Mistral OCR), then deterministically validated — required fields, arithmetic consistency, vendor matching, and duplicate detection — before landing in `VALIDATED`, `NEEDS_REVIEW`, or `DUPLICATE`, all in one synchronous request. A `NEEDS_REVIEW` document now goes somewhere real: `GET /review` lists the queue with the specific rule(s) that failed, `GET /review/{id}` shows the extracted fields next to the original document (via a short-lived, HMAC-signed file URL — no login needed to view it, the signature itself is the authorization), and a reviewer either corrects the fields (`POST /review/{id}/correct`, writing an auditable `review_events` row per field and moving the document to `VALIDATED`) or rejects it outright (`POST /review/{id}/reject`, to `REJECTED`). Nothing looks at AI confidence to decide any of this; see [`app/validation.py`](app/validation.py), [`app/vendor_matching.py`](app/vendor_matching.py), [`app/duplicate_detection.py`](app/duplicate_detection.py), and [`app/review.py`](app/review.py). The approval-routing step doesn't exist yet. See [`PLAN.md`](PLAN.md) for the full phase roadmap and current progress.

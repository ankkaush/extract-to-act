...
def evaluate_provider(provider_name: str, ground_truth: dict[str, dict]) -> dict:
    ...
    if not results:
        return {"provider": provider_name, "documents_evaluated": 0}
...
def render_report(evaluations: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("| Provider | Docs | Errors | Overall field accuracy | Avg confidence (correct) | "
                  "Avg confidence (wrong) | Avg latency (s) | Total cost | Line-item count match |")
    lines.append("|---|---|---|---|---|---|---|---|---|")
    ...
    lines.append(
        ...
        "`spike/README.md` for how it was built."
    )
    return "\n".join(lines)

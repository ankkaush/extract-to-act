...
**Blocked, not skipped:** the actual spike run against real providers needs Azure Document Intelligence, Mistral, and Anthropic credentials — none exist in this environment. Sample-invoice sourcing is resolved (synthetic, approved). The provider wrapper code itself is written against each SDK's documented shape but is unverified against a live account — treat the first real run as also testing this code.

...
**Also built:** every provider module is now split into a real-I/O `extract()` and a pure `parse_result()`/`parse_response()` that maps an SDK-response-shaped object to the normalized schema — tested in `spike/test_providers.py` against hand-built fake response objects, no SDK package or credentials needed. Plus `spike/test_formatting.py`, `spike/test_evaluate.py`, `spike/test_dataset_integrity.py` (guards the committed dataset against drift — e.g. a spec edited without regenerating its ground truth file), and `spike/test_run_spike.py` (the orchestrator's provider dispatch, per-call error handling, and budget-cap enforcement, with every provider's `extract()` faked out — the "happy path" that only manual no-samples/no-credentials checks had covered before). 68 tests total pass in the same lean environment CI uses, none requiring the `[spike]` extra.
**Blocked, not skipped:** the actual spike run against real providers needs Azure Document Intelligence, Mistral, and Anthropic credentials — none exist in this environment, and the project owner has explicitly deferred providing them. Everything achievable without credentials is now done; the remaining work is entirely the real comparison run itself.
**Completion criteria:** not met — pending credentials, then a real run + `docs/extraction-strategy.md` update with results.

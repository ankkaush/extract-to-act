...
Phase 4 (upload validation), Phase 6 (provider data handling), Phase 10/11 (auth on review/approval endpoints), and consolidated verification in Phase 14 (implemented): gitleaks run against full git history (not just the working tree) confirms no real secret has ever been committed — one known false positive (the documented dev-only default key in a README example) is suppressed via `.gitleaksignore` by exact fingerprint, not by disabling the rule; a prompt-injection adversarial fixture (`tests/test_security_hardening.py`) proves extracted text is handled as inert data through the real validation/vendor-matching code paths, not just by architectural claim; every review/approval endpoint's attribution field is confirmed non-optional at the API layer (`422` without it); the absence of any bank/IBAN/routing field anywhere in the extraction schema is now a regression-guarded assertion, not just an omission; and Mistral's actual API-tier data-training policy is disclosed in the README, not merely gestured at. A real gap was also found and fixed here, not just verified: `_attempt_extraction`/`_attempt_action`'s failure-reason strings (Phase 13) included the raw exception message from an external-provider call, which the "Secret-safe debugging practice" section above exists specifically to prevent — fixed to log the exception type only.

...
- Extraction is **not** a separate call — `POST /documents` runs it synchronously as part of the upload request and only returns once it's done. A recovery worker exists as of Phase 13 (`app/worker.py`) for a document that gets stuck mid-flight after a crash, but nothing schedules it to run automatically yet — that's Phase 17's deployment concern, not this endpoint's; see `app/routers/documents.py`.
...
- `POST /documents/{id}/action` — the only endpoint this phase adds. Requires `state == VALIDATED` and, if the document is at or above the approval threshold, an existing `approvals` row (`409` "awaiting approval..." otherwise). Writes an `accounting_actions` row `ATTEMPTED` before the write (idempotency-first, per `docs/reliability.md` scenario 3), retries the write with bounded backoff (Phase 13, `app/retry.py`), creates the actual payable record in the mock `ap_ledger_entries` table (`app/accounting.py`'s `MockAccountingProvider`), marks the `accounting_actions` row `CONFIRMED`, and moves the document `VALIDATED → ACTIONED → COMPLETED`. Best-effort notification (retried briefly, then logged and given up on — `notify_with_retry`) — a notification failure never blocks or unwinds the write; `notification_sent: false` in the response is the only sign it happened. A repeated call after `COMPLETED` returns the existing result rather than writing a second ledger entry (`accounting_actions.document_id` is unique). If the accounting write's retries are exhausted, the document moves to `FAILED`, an alert is sent, and the endpoint returns `502`.

Nothing calls this endpoint automatically — see `docs/adr/0003-worker-model.md`; no scheduler is wired up yet (Phase 17's deployment concern). A document left stuck `ACTIONED` by a crash between the `ATTEMPTED` and `CONFIRMED` writes is *not* auto-resumed by this endpoint itself (it just 409s as "not VALIDATED") — that's `app/worker.py`'s `recover_document()` (Phase 13), which resumes it directly rather than re-entering this route.

`DocumentActionOut`/`AccountingActionOut` shapes: see `app/schemas.py`.

### Phase 13 — Reliability & Recovery (implemented)
No new endpoint — this phase makes the existing synchronous steps (extraction, the accounting write) retry themselves with bounded exponential backoff before giving up (`app/retry.py`), and adds a separate recovery path for the case retrying-in-request can't cover: the app process itself dying mid-step. `app/worker.py`'s `run_once()` scans for documents stuck in `EXTRACTING`/`VALIDATING`/`ACTIONED` past a configurable timeout and resumes each from exactly where it left off — never reprocessed from `RECEIVED` — by calling the same internal `_attempt_extraction`/`_attempt_validation`/`_attempt_action` functions the request path uses. A document's `retry_count` bounds how many times the worker will attempt this before dead-lettering it (`FAILED` + a best-effort alert) — a second, outer layer of bounding beyond each call's own in-request retry attempts, so a sustained outage still terminates instead of the worker retrying forever. Nothing schedules `run_once()` to actually run yet (no cron, no loop wired into deployment) — that's Phase 17.

### Phase 16 — Observability & Business Metrics (implemented)
- `GET /dashboard` — the metrics summary, computed live from `documents`/`state_history`/`review_events` via aggregation queries, no separate metrics-writing system (`app/metrics.py`, `app/routers/dashboard.py`). `total_processed` excludes documents still actively in flight (`RECEIVED`/`EXTRACTING`/`VALIDATING`) — everything else, including a document currently sitting unresolved in `NEEDS_REVIEW`, already represents a real outcome. Genuinely measured: `straight_through_rate`/`review_rate` (did a document ever reach `NEEDS_REVIEW`), `correction_rate` (of the documents that did, how many were corrected rather than rejected), `average_processing_time_seconds` (from each terminal document's `RECEIVED` timestamp to its terminal one). Explicitly an estimate, never presented otherwise: `estimated_minutes_saved` — `straight_through_count × estimated_manual_minutes_per_document`, where the per-document assumption is a configurable setting (`ESTIMATED_MANUAL_MINUTES_PER_DOCUMENT`, default 8.0) returned in the same response so the assumption is never hidden behind the number.

`DashboardMetricsOut` shape: see `app/schemas.py`.

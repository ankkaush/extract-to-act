...
Test tiers are introduced starting Phase 2 (CI skeleton) and used by every subsequent phase per its own testing scope in `PLAN.md`. **Phase 15 (implemented):** `tests/test_e2e_real_provider.py` — two `@pytest.mark.real_api` tests, run manually and rarely, never in CI or a default `pytest` invocation. One proves the full pipeline still works against the live Mistral API (drift detection a fixture-only suite can't provide); the other is the "basic chaos/failure injection" this phase names — a real, live authentication failure, proving the failure path (retry, dead-letter, notification) behaves exactly as the fixture-based tests already proved for a *simulated* failure, and specifically that Phase 14's exception-message fix holds against a genuine SDK exception, not just a hand-rolled one. Two real calls total for this phase, well inside the whole-project budget above. A chaos test that actually kills a running process was deliberately not built — `docs/reliability.md`'s own stated approach (writing a stuck in-flight state directly to the database, already exercised by Phase 13's worker tests) is the documented, preferred technique precisely because it's deterministic where process-killing isn't.
